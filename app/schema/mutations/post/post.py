...
class PostUploadMutation(graphene.Mutation):
    ...
    def mutate(cls, _, info, title, text):
        ...
        return PostUploadMutation(
            ResponseMessageField(is_success=True, message="Upload successful")
        )
    ...
    def mutate(cls, _, info, token, post_id):
        ...
        if post and post.author == username:
            ...
            return PostDeleteMutation(
                ResponseMessageField(is_success=True, message="Delete successful")
            )
        elif post is None:
            return PostDeleteMutation(
            ...
            return PostDeleteMutation(
                ResponseMessageField(
                    is_success=False, message="No authority to delete this post"
                )
            )
